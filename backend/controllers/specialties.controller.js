...
exports.createNewSpeicalty = (req, res) => {
  ...
  img = filepath.replace('..\\frontend\\src\\assets', 'assets');
...
exports.editSpecialty = (req, res) => {
  ...
  SpeicaltiesSchema.updateOne(
    ...
    function (err, result) {
      ...
      res
        .status(200)
        .send({status: 'success', data: {}, msg: 'Specialty modified.'});
    }
  );
  // }
};
...
exports.deleteSpecialty = (req, res) => {
  SpeicaltiesSchema.deleteOne({_id: req.body._id}, function (err, results) {
    ...
    //     fs.unlink("../frontend/src/"+req.body.img);
    //     fs.unlink("../frontend/src/" + req.body.img, (err) => {
    //         if (err) {
